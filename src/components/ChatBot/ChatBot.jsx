...
const ChatBot = ({ insideShell }) => {
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            if (!apiKey) {
                throw new Error('API key not configured. Set VITE_MISTRAL_API_KEY in .env');
            }
                ...
                <div className="chatbot-window" style={{ bottom: '110px', right: '0' }}>
                    ...
                    <div className="chatbot-messages">
                        {messages.map((msg, idx) => (
                            <div key={idx} className={`chatbot-message chatbot-message-${msg.type}`}>
                                <div className="chatbot-message-content">
                                    {msg.type === 'bot' && (
                                        <img src={hatIcon} alt="Sombrero mágico" className="chatbot-message-icon" />
                                    )}
                                    <span>{msg.text}</span>
                                </div>
                            </div>
